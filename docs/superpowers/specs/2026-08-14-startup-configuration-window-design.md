...
Window/buffer state (`FWindowOpen`, `FStartupBuffer`, drop counter, `MinLevel` snapshot) is guarded by the class-level lock, exactly like other class state. `CompleteConfiguration` may be called from any thread; the idempotent check-and-close plus the buffer snapshot-and-clear happen under that class lock as one atomic step.

Provider dispatch is handled separately: the non-default provider list is snapshotted (copied) under the instance monitor, the monitor is released, and the actual replay -- looping the buffered entries against the snapshot, plus the drop-warning entry if any -- runs with **neither** lock held. This is a deliberate departure from the original design (which held the instance monitor for the whole replay): a bound provider's `Log` can block the calling thread for an unbounded time under backpressure -- e.g. an async provider's queue is full and its worker thread is itself waiting on `TThread.Synchronize` back onto the very thread that is running `CompleteConfiguration`. Dispatching outside both locks removes that deadlock class without requiring every provider to promise it never blocks. Accepted trade-off: with no lock serializing replay against concurrent live dispatch after the close, a live entry can reach a provider before, after, or interleaved with the tail of the replay -- entries are still never lost or duplicated (the snapshot-and-clear under the class lock remains the sole source of truth for what was buffered), but relative order across the close boundary is no longer guaranteed; each entry's `Timestamp` still records its true order. See `docs/CONFIGURATION.md`, "Startup & Configuration Window", for the same trade-off documented for API consumers.
